...
static int detect_network_iface(void *ptr, const char *d_name)
{
    ...
    // ifaceNum 4
    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.4", rootdir, d_name);
    if (!access(pl->path, R_OK))
    {
        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.4/GobiQMI", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;

        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.4/usbmisc", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;

        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.4/usb", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;
    }

    // ifaceNum 8
    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.8", rootdir, d_name);
    if (!access(pl->path, R_OK))
    {
        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.8/usbmisc", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;

        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.8/usb", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;
    }
    dbg_time("cannot find GobiQMI/usbmic/usb in path %s, subdir %s:1.4 or %s:1.8", rootdir, d_name, d_name);
    return -1;
}
...
BOOL qmidevice_detect(char *qmichannel, char *usbnet_adapter, unsigned bufsize, int *pbusnum, int *pdevnum) {
    ...
    while ((ent = readdir(pDir)) != NULL)  {
        ...
        if (detect_network_iface(pl, ent->d_name))
            continue;
